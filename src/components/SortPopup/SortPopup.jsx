...
export const SortPopup = React.memo(function SortPopup({activeSortType, onClickSortType, items}) {
                ...
                <div className="sort__popup">
                    <ul>
                        {items && items.map((item, i) => {
                            return <li onClick={() => selectPopupItem(item)}
                                       className={activeSortType === item.type ? 'active' : ''}
                                       key={`${item.type}_${i}`}>{item.name}</li>
                        })}
                    </ul>
                </div>
            }
